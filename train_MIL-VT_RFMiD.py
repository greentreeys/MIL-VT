...
from models.FinetuneVTmodels import FineTuneTransformer_Distil2Plaine
from utils import *
...
def main():
    ...
    """Basic Setting"""
    data_path = r'/extracephonline/medai_shirlyyu/data/RFMiD/Training_Set/Image/'
    csv_path = r'/extracephonline/medai_shirlyyu/data/RFMiD/Training_Set/CSV/'
    save_model_path = r'/extracephonline/medai_shirlyyu/data/RFMiD/Training_Set/PytorchModel/'
    csvName = csv_path + 'RFMiD_Training_Labels.csv'
    ...
    balanceFlag = False  #balanceFlag is set to True to balance the sampling of different classes
    fundusWeightFlag = True  #whether to use the fundus pretrain weight
    ...
    model = FineTuneTransformer_Distil2Plaine(base_model, MODEL_PATH_finetune, num_classes=n_classes, fundusWeightFlag=fundusWeightFlag)
    model = model.cuda()
    if len(gpu_ids) >= 2:
    ...
    if balanceFlag == True:
        ...
        train_loader = DataLoader(dataset_train, batch_size,
                              num_workers=8,  drop_last=True, shuffle=True) #shuffle=True,
    valid_loader = DataLoader(dataset_valid, batch_size,
    ...
    for epoch in range(start_epoch, max_epoch + 1):
        ...
        if F1_val > best_perform and debugFlag == False:
            best_perform = F1_val
        ...
        if epoch>0 and (epoch) % save_fraq == 0 and debugFlag == False:
            ...
            saveCheckPointName = savemodelPrefix + str(epoch) + '_' + str(F1_val) + '.pth.tar'
            utils.save_checkpoint({
                'state_dict': state_dict,
                'epoch': epoch,
                'lr': optimizer.param_groups[0]['lr'],
...
def train(epoch, model, criterion, optimizer, train_loader, max_epoch,  tbWriter):
    ...
    precision = metrics.precision_score(gts, preds)
...
def val(epoch, model, criterion, val_loader, max_epoch, tbWriter):
    ...
    tbWriter.add_scalar('F1/valid', F1, epoch)
    tbWriter.add_scalar('Recall/valid', recall, epoch)
    tbWriter.add_scalar('Precision/valid', precision, epoch)
    tbWriter.add_scalar('Loss/valid', losses.avg, epoch)


    end_time = time.time()
    print('-------- Epoch {}/{}'.format(epoch, max_epoch))
    print('-------- Val:    Acc %0.3f,  AUC %0.3f, F1 %0.3f, Recall %0.3f, Precision %0.3f, loss %.6f, time %3.2f'
        % (accuracy, AUC, F1, recall, precision, losses.avg,  end_time - start_time))
    print('=============================================================')
    return AUC, F1
...
def test(epoch, model, criterion, test_loader,  tbWriter):
    ...
    end_time = time.time()
    print( 'TestSet:  Acc %0.3f,  AUC %0.3f, F1 %0.3f, Recall %0.3f, Precision %0.3f, time %3.2f'
        % (accuracy, AUC, F1, recall, precision,  end_time - start_time))
    print('=============================================================')
    return AUC, F1
